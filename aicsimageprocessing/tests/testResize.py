...
from aicsimageprocessing.resize import resize, resize_to
...
class TestImgResize(unittest.TestCase):
    def test_resizeOutput(self):
        ...
        self.assertEqual(
            resize(rand_cube, 2).shape,
            tuple([4] * n_dim),
            "Arbitrary dimension scaling",
        )
        # also takes in list for each dimension
        self.assertEqual(
            resize(rand_cube, [2] * n_dim).shape,
            tuple([4] * n_dim),
            "Individual dimension scaling",
        )
        # works for floats and zeros
        self.assertEqual(
            resize(test_cube, (1.5, 0.5, 0)).shape, (3, 1, 0), "Scale by floats or zero"
        )
    ...
    def test_resizeInputs(self):
        ...
        self.assertEqual(
            resize(test_cube, 3, method="bilinear").shape, (9, 9, 9), "Resize Bilinear"
        )
        self.assertEqual(
            resize(test_cube, 3, method="cubic").shape, (9, 9, 9), "Resize Cubic"
        )
    ...
    def test_resizeToOutSize(self):
        ...
        with self.assertRaises(
            ValueError, msg="out_size must be the same length as the input size"
        ):
    ...
    def resizeToMethods(self):
        test_cube = np.empty((3, 3, 3))
        self.assertEqual(
            resize_to(test_cube, (9, 9, 9), method="bilinear").shape,
            (9, 9, 9),
            "Resize_to Bilinear",
        )
        self.assertEqual(
            resize_to(test_cube, (9, 9, 9), method="cubic").shape,
            (9, 9, 9),
            "Resize_to Cubic",
        )
        # only accept certain methods
    ...
    def test_resizeToOutput(self):
        ...
        for t in range(10):
            ...
            self.assertEqual(
                ...
                "Random Resize_to {} -> {}, got {}".format(
                    arr_input, arr_output, out.shape
                ),
            )

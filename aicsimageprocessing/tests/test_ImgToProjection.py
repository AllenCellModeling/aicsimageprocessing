...
from aicsimageprocessing.imgToProjection import imgtoprojection


class TestImg2Projection(unittest.TestCase):
    def test_arrayInput(self):
        self.assertEqual(
            imgtoprojection(np.empty((1, 3, 4, 5))).shape, (3, 4, 5), "4d array input"
        )
        self.assertEqual(
            imgtoprojection(np.empty((10, 3, 4, 5))).shape,
            (3, 4, 5),
            "10 channel input",
        )
        ...
        self.assertEqual(
            imgtoprojection([np.empty((3, 4, 5))] * 2).shape,
            (3, 4, 5),
            "List of 3d arrays",
        )
        self.assertEqual(
            imgtoprojection([np.empty((4, 5))] * 2).shape,
            (3, 4, 5),
            "List of 2d arrays",
        )
    ...
    def test_projectionMethods(self):
        ...
        self.assertEqual(
            imgtoprojection(test_cube, proj_method="max").shape,
            (3, 5, 5),
            "Max projection",
        )
    ...
    def test_colorMethods(self):
        ...
        self.assertTrue(
            (
                imgtoprojection(np.ones((1, 5, 5, 5)), colors=[[255, 255, 255]])[0] == 1
            ).all(),
            "Scaling color array",
        )
        self.assertTrue(
            (imgtoprojection(test_cube, colors=[[0, 1, 1]])[0] == 0).all(),
            "Color cancelling with array",
        )

    def test_colorAdjust(self):
        self.assertTrue(
            (
                imgtoprojection(np.random.random((1, 5, 5, 5)), global_adjust=True)[0]
                > 254
            ).any(),
            "Global color adjust",
        )
    ...
    def test_projectAll(self):
        test_cube = np.empty((1, 5, 5, 5))
        self.assertEqual(
            imgtoprojection(test_cube, proj_all=True).shape, (3, 10, 10), "Project All"
        )

...
class TestImgCenter(unittest.TestCase):
    def getRandTest(self):
        ...
        test[tuple([slice(None, None)] + [slice(4 + m, 7 + m) for m in moves[1:]])] = 1
        return (test, moves)
    ...
    def test_getCenterCalc(self):
        ...
        self.assertEqual(
            moves,
            calc_moves,
            "Calculated moves {} should equal {}".format(calc_moves, moves),
        )
    ...
    def test_imgCenterMoves(self):
        ...
        self.assertTrue(
            np.array_equal(*res), "Should move multiple images the same amount"
        )
    ...
    def test_cropAllInputs(self):
        with self.assertRaises(
            ValueError, msg="crop_all requires an iterable as input"
        ):
        ...
        with self.assertRaises(ValueError, msg="Image shapes must be equal"):
            images = [self.getRandTest()[0] for _ in range(2)] + np.empty(
                [randrange(1, 10) for _ in range(4)]
            )
            crop_all(images)
    ...
    def test_cropAllOutSize(self):
        ...
        self.assertTrue(
            all(shape == img.shape for img in out_images[1:]),
            "Images are same size after cropping",
        )
    ...
    def test_cropAllAxis(self):
        for t in range(3):
            ...
            self.assertTrue(
                test_images[0].shape[a] == out_images[0].shape[a],
                "Does not crop on non-specified axis",
            )
    ...
    def test_cropAllOutCOM(self):
        ...
        for img in out_images:
            ...
            self.assertTrue(
                all(
                    float(mark) / length == 0.5
                    for mark, length in zip(mark[1:], img.shape[1:])
                ),
                "Mark remains at midpoint after cropping",
            )
